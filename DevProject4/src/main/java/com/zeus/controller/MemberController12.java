 ...
 *   로 구분하도록 할 수 있다.
 * 
 * <form:select>은 <select>태그와 같이 multiple 속성을 사용할 수 있다.
 * <form:select>은 <form:option>, <form:options>라는 하위 태그가 존재한다.
 * <form:options>는 <form:select>와 같이 속성을 사용할 수 있다.
 * -----------------------------------------------------------------
 * <form:select path="carList" multiple="false">
		<form:option value="" label="=== 선택해 주세요 ===" />
		<form:options items="${carCodeList}" itemValue="value"
			itemLabel="label" />
	</form:select></td>
 * -----------------------------------------------------------------
 * 처럼 표현할 수 있다.
...
public class MemberController12 {
	...
	// 입력처리_2 : multiple="true"의 값을 처리할 수 있다. 

...
/*
 * Model 객체에 자바빈즈 클래스 객체를 값으로만 전달할 때는 
 * 뷰에서 참조할 이름을 클래스명의 첫번째 글자를 소문자로 변환하여 처리한다.
 */
@Log
@Controller
public class MemberController1 {
	...
	public String model05(Model model) {
		...
		cal.set(Calendar.MONTH, 10);
		cal.set(Calendar.DAY_OF_MONTH, 7);
		member.setDateOfBirth(cal.getTime()); //Member dateOfBirth를 set
		
		String introduction = "안녕하세요.\n반갑습니다."; 
		member.setIntroduction(introduction); //set

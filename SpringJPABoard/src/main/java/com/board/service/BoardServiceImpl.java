...
public class BoardServiceImpl implements BoardService {
	...
	public Board read(Long boardNo) throws Exception {
		return repository.getOne(boardNo); //인수에 ID를 지정하여 엔터티를 하나를 얻어온다. like select * from .. where = id
	}

	@Override
	@Transactional
	public void modify(Board board) throws Exception {
		Board boardEntity = repository.getOne(board.getBoardNo()); //인수에 ID를 지정하여 엔터티를 하나를 얻어온다.
	...
	public void remove(Long boardNo) throws Exception {
		repository.deleteById(boardNo); //인수에 지정된 ID 의 엔티티를 데이터베이스에서 삭제한다. like delete from .. where = id
	}
	 ...
	 *  : 지정된 Sort.Orders에 대한 새 정렬을 만듭니다. 
	 *  Parameters: direction(must not be null), properties(must not be null)
	 */
		return repository.findAll(Sort.by(Direction.DESC, "boardNo")); 
		//return에 대한 해석 : Sort.by static메서드에 따라 boardNo를 기준으로 내림차순 한 후 전체 엔티티를 정리한 List객체를 돌려준다.
	}
